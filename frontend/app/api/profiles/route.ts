...
export async function GET() {
    try {
        ...
        const { data: profiles, error } = await supabaseService
            .from('profiles')
            .select('wallet_address, name, birthYear, gender, interests, photoUrl, wallet_verified, email_verified, farcaster_verified') // ✅ Added farcaster_verified
        ...
        // ✅ Log sample photoUrls and verification status for debugging (first 3 profiles)
